  ...
  function setupAutoConsent () {
    ...
    class AutoConsent {
      ...
      async _start () {
        ...
        console.log('cmps found')
        console.log(cmps)
        console.log(this.config.webmunkCondition)
          ...
          for (const cmp of cmps) {
            ...
            popupLookups.push(this.waitForPopup(cmp).then((isOpen) => {
              if (isOpen) {
                ...
                console.log('cmpDetected')
                console.log(this.foundCmp)
                console.log(this.config.webmunkCondition)

                if (this.config.webmunkCondition === 'organic') {
                  console.log('[Uniform Cookie UI] TODO: Add listeners for default actions...')
                  console.log(this.config)
                } else { // Show uniform interface
                  this.sendContentMessage({
                    type: 'popupFound',
                    cmp: cmp.name,
                    url: window.location.href
      ...
      generateUniformHtml (condition) {
        ...
        cookieSettings += '    <button style="border: 1px solid #8f8f8f; '

        if (condition.includes('monosettings')) {
          cookieSettings += 'background-color: #757575; color: #FFFFFF;'
        } else {
          cookieSettings += 'background-color: #70AC47; color: #FFFFFF;'
        }

        cookieSettings += '" id="uniform_cookie_ui_button_cookie_settings">Cookie Settings</button>'

        cookieSettings += '    <div id="uniform_cookie_ui_section_cookie_settings" class="uniform_cookie_ui_hide_element">'
        ...
        categories.forEach(function (category) {
          ...
          cookieSettings += '      <div class="uniform_cookie_ui_category">'
          cookieSettings += '        <label>'
          cookieSettings += '          <input type="checkbox" name="uniform_cookie_ui_category" value="' + category[1] + '" class="' + categoryClasses + '" ' + disabledAttr + '>'
          cookieSettings += '          <span>' + category[0] + '</span>'
          cookieSettings += '        </label>'
          cookieSettings += '      </div>'
        })
        ...
        if (availableConsents.length === 0) {
          cookieSettings += '          <input type="checkbox" id="uniform_cookie_ui_category_select_all" value="all" disabled checked>'
        } else {
          cookieSettings += '          <input type="checkbox" id="uniform_cookie_ui_category_select_all" value="all">'
        }

        cookieSettings += '          <span class="uniform_cookie_ui_category_select_all">Select All</span>'
        cookieSettings += '        </label>'
        cookieSettings += '      </div>'

        if (availableConsents.length === 0) {
          cookieSettings += '      <p class="uniform_cookie_ui_message_limited_selection">This site does not allow you to select the types of cookies that you may receive.</p>'
        }

        cookieSettings += '      <button style="border: 1px solid #8f8f8f; background-color: #70AC47; color: #FFFFFF;" id="uniform_cookie_ui_button_accept_selected">Accept Selected Cookies</button>'
        cookieSettings += '    </div>'

        const cookieAcceptAll = '    <button style="border: 1px solid #3f7b18; background-color: #70AC47; color: #FFFFFF;" id="uniform_cookie_ui_button_accept_all">Accept All Cookies</button>'
        const cookieRejectAll = '    <button style="border: 1px solid #3f7b18; background-color: #70AC47; color: #FFFFFF;" id="uniform_cookie_ui_button_reject_all">Reject All Cookies</button>'

        const sections = condition.split('-')

        sections.forEach(function (section) {
          if (section === 'settings' || section === 'monosettings') {
            htmlCode += cookieSettings
          } else if (section === 'accept') {
            htmlCode += cookieAcceptAll
          } else if (section === 'reject') {
            htmlCode += cookieRejectAll
          }
        })
      ...
      showUniformInterface () {
        ...
        console.log('[Uniform Cookie UI] Build standard cookie interface for:')
        console.log(this.config)
        ...
        const htmlCode = this.generateUniformHtml(this.config.webmunkCondition)
  ...
  window.registerModuleCallback(function (config) {
    ...
    if (window.location === window.parent.location) {
      ...
      const tagCookieElements = function () {
        console.log('[Uniform Cookie UI] Looking for UI elements...')
        ...
        for (const selector in cookieLabels) {
          ...
          console.log('[Uniform Cookie UI] Searching for ' + rule + '... ' + matches.length + ' found.')

          if (matches.length > 0) {
            console.log('[Uniform Cookie UI] Found ' + selector + ': ' + matches.length)
        ...
        cookieClickLabels.forEach(function (clickLabel) {
          ...
          $(rule).on('click', function (event) {
            ...
            console.log('[Uniform Cookie UI] Clicked ' + clickLabel + '.')
        ...
        for (const selector in cookieLabels) {
          ...
          console.log('[Uniform Cookie UI] Searching for ' + rule)
